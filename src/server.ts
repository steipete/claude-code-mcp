...
export class ClaudeCodeServer {
  ...
  constructor(options?: {
    ...
    if (
      ...
      if (typeof (this.server as any)?.setRequestHandler !== "function") {
        console.error(
          "!!!!!!!!!!!! SERVER FALLBACK ACTIVATED (setRequestHandler missing) !!!!!!!!!!!!",
        );
        this.server = {
  ...
  private async _initializeClaudeCliVersion(): Promise<void> {
    ...
    try {
      ...
      const { stdout } = await this.spawnFunction(this.claudeCliPath, ["--version"], {
        timeout: 5000,
      }); // 5s timeout
  ...
  private setupToolHandlers(): void {
    ...
    if (timeoutSecondsEnv) {
      ...
      if (!isNaN(parsedTimeout) && parsedTimeout > 0) {
        ...
        debugLog(
          `[Config] Using custom Claude CLI timeout: ${executionTimeoutSeconds} seconds from CLAUDE_CLI_TIMEOUT_SECONDS.`,
        );
      } else {
        debugLog(
          `[Warning] Invalid value for CLAUDE_CLI_TIMEOUT_SECONDS: "${timeoutSecondsEnv}". Using default: ${defaultTimeoutSeconds} seconds.`,
        );
      }
    } else {
    ...
    this.server.setRequestHandler(
      CallToolRequestSchema,
      async (args: any, call: any): Promise<ServerResult> => {
        ...
        try {
          ...
          if (
            ...
            throw new McpError(
              ...
              `Claude CLI command timed out after ${executionTimeoutMs / 1000}s. Details: ${errorMessage}`,
            );
          }
