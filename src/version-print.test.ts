...
let localSpawnAsyncMock: Mock<
  (...args: any[]) => Promise<{ stdout: string; stderr: string; exitCode: number }>
>;
...
const findVersionCall = (calls: any[][]) => {
  return calls.find(
    (call: any[]) =>
      ...
      call[0].includes("Claude Code MCP server running on stdio") &&
      typeof call[1] === "string" &&
      call[1].startsWith("claude_code v"),
  );
};
...
describe("Version Print on First Use", () => {
  ...
  beforeEach(async () => {
    ...
    localSpawnAsyncMock.mockResolvedValue({
      stdout: "1.0.0-test-version",
      stderr: "",
      exitCode: 0,
    });
  ...
  it("should print version and startup time only on first use", async () => {
    ...
    const startupLogCall = consoleLogSpy.mock.calls.find(
      (call: any[]) => call[0] === "Claude Code MCP server running on stdio",
    );
    ...
    expect(localSpawnAsyncMock).toHaveBeenCalledWith(
      expect.any(String),
      ["--version"],
      expect.anything(),
    );
    expect(consoleErrorSpy).not.toHaveBeenCalledWith(
      expect.stringMatching(/claude_code v[0-9]+\.[0-9]+\.[0-9]+/),
    );
