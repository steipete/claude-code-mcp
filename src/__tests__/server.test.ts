...
const ServerTestMockSdkServer = vi.fn(function () {
  const instance = {
  ...
  describe("ClaudeCodeServer class", () => {
    it("should initialize with correct settings and fetch CLI version", async () => {
      ...
      expect(spawnAsyncMock).toHaveBeenCalledWith(
        "/fake/cli/path/claude",
        ["--version"],
        expect.objectContaining({ timeout: TEST_CLI_INIT_TIMEOUT_MS }),
      );

      const internalServer = server.server as any;
      expect(
        internalServer.__isServerTestFileMock,
        "SDK Server mock from server.test.ts should be used",
      ).toBe(true);
    });
    ...
    it("should handle initialization failure if findClaudeCli throws", async () => {
      ...
      findClaudeCliMock.mockImplementation(() => {
        throw cliError;
      });
    ...
    it("should NOT activate server fallback when SDK global mock is correctly applied", async () => {
      ...
      expect(consoleErrorSpy).not.toHaveBeenCalledWith(
        expect.stringContaining("SERVER FALLBACK ACTIVATED"),
      );
    });
    ...
    it("should set up request handlers for ListToolsRequest and CallToolRequest", async () => {
      ...
      const listToolsCall = setRequestHandlerCalls.find(
        (call: [any, any]) => call[0] === ListToolsRequestSchema,
      );
      const callToolCall = setRequestHandlerCalls.find(
        (call: [any, any]) => call[0] === CallToolRequestSchema,
      );
  ...
  describe("Tool handler implementation", () => {
    ...
    beforeEach(async () => {
      ...
      spawnAsyncMock.mockImplementation(async (command: string, args: string[]) => {
        ...
        return {
          stdout: "CLI command successful from tool handler default mock",
          stderr: "",
          exitCode: 0,
        };
      });
    ...
    it("should handle CallToolRequest and execute claude CLI", async () => {
      ...
      spawnAsyncMock.mockResolvedValueOnce({
        stdout: "CLI command successful",
        stderr: "",
        exitCode: 0,
      });

      const result: any = await callToolHandler(
        { params: { name: "claude_code", arguments: mockArgs } },
        {} as any,
      ); // Reverted to any
    ...
    it("should handle non-existent workFolder by using default and warning", async () => {
      ...
      expect(debugLogMock).toHaveBeenCalledWith(
        expect.stringContaining(
          "Specified workFolder does not exist: /non/existent/folder. Using default: /fake/default/home",
        ),
    ...
    it("should handle errors from the Claude CLI execution", async () => {
      ...
      try {
        ...
        expect(e.message).toContain(
          "Claude CLI execution failed: CLI execution failed with error output",
        );
