...
const MockSdkServer = vi.fn(function () {
...
describe("Error Handling Tests", () => {
  ...
  describe("CallToolRequest Error Cases", () => {
      ...
      await expect(
        ...
        expect.objectContaining({
          message: expect.stringMatching(
            /Claude CLI command timed out|Process execution timed out after/,
          ),
    ...
    it("should include CLI error details in error message", async () => {
      ...
      const errorFromSpawn: any = new Error(
  ...
  describe("Process Spawn Error Cases (testing spawnAsync directly via spy)", () => {
    it("should handle spawn ENOENT error", async () => {
      ...
      await expect(actualUtils.spawnAsync("nonexistent-command", [])).rejects.toThrow(
